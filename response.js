var D = require("DBManager.js")("D"); 
	...
	function lyric(r) {
    var str = r.msg.replace("/가사 ", "");
    var title = str.split("-")[0];
    var artist = str.split("-")[1];
    ...
    var songs = org.jsoup.Jsoup.connect("http://lyrics.alsong.co.kr/alsongwebservice/service1.asmx").header("Content-Type", "text/xml;charset=utf-8").requestBody(xml).post().select("ST_GET_RESEMBLELYRIC2_RETURN");

    r.replier.reply(str+title+artist+songs);
}
